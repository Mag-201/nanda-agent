...
def handle_message(message):
    ...
    # 如果用户说“stock”或“股票”，就触发股票查询
    if ("stock" in lower) or ("股票" in lower):
        syms = extract_stock_symbols(user_input)
        if not syms:
            return "没有找到股票代码，请输入如 AAPL、TSLA 这样的代码"
        return "\n".join(get_stock_price(s) for s in syms)

    # 其余的继续走原本逻辑
    return "（这里放你原来的 Claude/其他逻辑）"
